...
<template>
  <div class="space-y-4">
    ...
    <!-- 로딩 상태 -->
    <section v-if="ledger.loading" class="bg-white rounded-xl border border-slate-100 py-[52px]">
    ...
    <section v-else class="bg-white rounded-xl border border-slate-100 divide-y divide-slate-100">
      ...
      >
        <div class="min-w-0">
          <p class="text-[14px] font-semibold text-slate-900 truncate flex items-center">
            {{ item.personName }}
            <span
              class="ml-1 rounded-full px-[6px] py-[2px] text-[11px] font-medium text-white relative -top-[1px]"
